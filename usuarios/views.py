...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if alumno:
            ...
            nombre_completo = user.get_full_name().strip() or user.dni
...
class CarreraPublicListView(ListView):
    model = Carrera
    template_name = 'usuarios/carrera_public_list.html'
    context_object_name = 'carreras'
    queryset = Carrera.objects.order_by('nombre')


class CarreraPublicDetailView(DetailView):
    model = Carrera
    template_name = 'usuarios/carrera_public_detail.html'
    context_object_name = 'carrera'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['materias'] = self.object.materias.order_by('nombre')
        return context
...
class OfertaAcademicaView(RolRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not alumno or not alumno.carrera:
            return Materia.objects.none()
        return Materia.objects.filter(carrera=self.request.user.alumno.carrera).select_related('carrera')
